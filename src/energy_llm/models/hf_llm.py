...
from energy_llm.models.model_profiles import (
    DEFAULT_MODEL_ALIAS,
    MODEL_PROFILES,
    ModelProfile,
)
...
class HFLLM:
    def __init__(
        ...
        self.alias = model_id if model_id in MODEL_PROFILES else None
        self.profile: Optional[ModelProfile] = MODEL_PROFILES.get(model_id, None)
        ...
        self.num_hidden_layers = getattr(self.model.config, "num_hidden_layers", len(self.layers))
    ...
    def _resolve_backbone(self):
        ...
        raise AttributeError(
            f"No se pudo resolver backbone para {self.model.__class__.__name__}. "
            f"Attrs disponibles: {dir(self.model)}"
        )
    ...
    def _resolve_layers(self):
        ...
        raise AttributeError(
            f"No se pudo resolver lista de layers en backbone {self.backbone.__class__.__name__}. "
            f"Attrs disponibles: {dir(self.backbone)}"
        )
    ...
    def _print_load_summary(self):
        ...
        print(f"[HFLLM] Loaded '{self.model_id}' on {self.device} ({quant_desc})")

        if self.profile is not None and self.load_in_4bit_requested and not self.profile.safe_4bit:
            print(f"[HFLLM] Warning: 4-bit request disabled because safe_4bit=False for alias '{self.alias}'.")

        print(
            f"[HFLLM] Resolved {self.n_layers} layers | "
            f"hidden_size={self.hidden_size} | "
            f"profile={'yes' if self.profile is not None else 'no'} | "
