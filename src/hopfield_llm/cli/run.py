...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = subparsers.add_parser(
        "analyze",
        help="Aggregate trajectory outputs into analysis JSON (CPU-only)",
    )
    p.add_argument(
        "--trajectories", required=True,
        help="Path to directory containing trajectory .npz and .json files",
    )
    ...
    p.add_argument(
        ...
        help="Metric for scalar hallucination score",
    )
    p.add_argument(
        "--score-aggregation", default="mean", dest="score_aggregation",
        choices=["mean", "max", "weighted"],
    )
    p.add_argument(
        "--signal-zone", type=int, nargs=2, default=None, dest="signal_zone",
        metavar=("START", "END"),
        help="Layer slice [start, end) for scoring (default: all layers)",
    )

    # ── Stage 4 — visualization ──────────────────────────────────────
    p = subparsers.add_parser(
        "visualize",
        help="Generate plots from analysis JSON (CPU-only)",
    )
    p.add_argument("--analysis", required=True, help="Path to analysis JSON file")
    p.add_argument("--output", required=True, help="Output directory for plots")

    # ── Convenience — run full experiment from config ─────────────────
    p = subparsers.add_parser(
        "run-experiment",
        help="Run all pipeline stages from an experiment YAML config",
    )
    ...
    p.add_argument("--no-4bit", action="store_true", help="Override: disable 4-bit")
    p.add_argument(
        "--shard-id", type=int, default=None, dest="shard_id",
        help="Shard index (for SLURM array jobs)",
    )
    p.add_argument(
        "--num-shards", type=int, default=None, dest="num_shards",
        help="Total shards (overrides config)",
    )
    p.add_argument(
        "--skip-banks", action="store_true", dest="skip_banks",
...
def _run_experiment(args: argparse.Namespace) -> int:
    """Execute all pipeline stages from an experiment config."""
    ...
    # Environment auto-detection for device default
    env = load_env_config()
    device = args.device or env.device

    # Determine shard_id (single-machine default: 0)
    ...
    try:
        ...
        run_trajectory(
            ...
            diagnostic_subset=cfg.diagnostic_subset,
            shard_id=shard_id,
        ...
        # Stage 3: Analyze (only if single-shard or all shards complete)
        if cfg.num_shards <= 1:
            analyze_trajectories(
                traj_dir=str(traj_dir),
                output=str(analysis_path),
                score_metric=cfg.score_metric,
                score_aggregation=cfg.score_aggregation,
            ...
            from hopfield_llm.utils.logging import get_logger
            get_logger("cli").info(
                "Multi-shard mode (shard %d/%d): skipping analysis/visualization. "
